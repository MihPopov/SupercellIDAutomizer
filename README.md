...
Про ввод `club_tag`:
- Ввод `club_tag` в поле поиска клуба делается прямым набором после принудительного переключения окна на EN-раскладку (`00000409`), чтобы символ `#` не превращался в `№`. Если прямой набор недоступен, используется fallback через буфер обмена (`Ctrl+V`).
